...
class ReportPeriodService {
  async getReportingPeriods(pageNumber = 1, pageSize = 10) {
    return await axios
      .get(`${HOST}/api/reportingPeriod?pageNumber=${pageNumber}&pageSize=${pageSize}`, {
        headers: {
  ...
  async getReportingPeriodById(id) {
    return await axios
      .get(`${HOST}/api/reportingPeriod/${id}`, {
        headers: {
          Authorization: `Bearer ${TOKEN}`,
        },
      })
